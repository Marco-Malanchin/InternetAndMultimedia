...
if dim > 255
    ...
    mystr= mystr(1:255); %prendo solo i primi 255 caratteri, matlab parte da 1 non da 0!
end
...
write(C, length(mystr), "uint8"); %riprendo la lunghezza della stringa(ora è al massimo 255) e converto questo numero in un byte rappresentato da 8 cifre binarie

% Send the bytes corresponding to the string characters
write(C, mystr, "uint8"); % mando i caratteri della mia stringa sottoforma di Byte

% Read the bytes returned by the server
uppercasestring = read(C, length(mystr), "uint8") ; %leggo la risposta del server e leggo esattamente lo stesso numero di byte che ho mandato (lengt(mystr))
